...
# from opssdkcore.logs import ins_log
...
class Application(tornadoApp):
    ...
    def __init__(self, handlers=None, default_host="", transforms=None, **settings):
        ...
        if configs.can_import:
            ...
            self.ins_log = configs.get("ins_log")
            self.ins_log.write_log('info', '%s' % options.progid)
    ...
    def start_server(self):
        ...
        """
        try:
            self.ins_log.write_log('info', 'progressid: %(progid)s' % dict(progid=options.progid))
            self.ins_log.write_log('info', 'server address: %(addr)s:%(port)d' % dict(addr=options.addr, port=options.port))
            self.ins_log.write_log('info', 'web server start sucessfuled.')
            self.io_loop.start()
            ...
            self.ins_log.write_log('error', '%(tra)s' % dict(tra=traceback.format_exc()))
